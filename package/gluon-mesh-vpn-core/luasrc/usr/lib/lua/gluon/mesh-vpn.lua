...
function M.get_provider(name)
	return require('gluon.mesh-vpn.providers.' .. name)
end

function M.get_provider_names()
	local out = {}

	for _, v in ipairs(util.glob('/lib/gluon/mesh-vpn/providers/*')) do
...
function M.get_active_provider()
	-- Active provider is the provider in use by the currently
	-- active site / domain
