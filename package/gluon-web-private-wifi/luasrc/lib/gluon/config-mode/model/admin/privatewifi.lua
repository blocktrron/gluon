...
if platform.supports_wpa3() then
...
if platform.supports_mfp(uci) then
	mfp:value("1", translate("Optional"))
	mfp:value("2", translate("Enabled"))
end
...
function f:write()
	util.foreach_radio(uci, function(radio, index)
		...
		if enabled.data then
			...
			-- hostapd-mini won't start in case 802.11w is found in
			-- the configuration
			if platform.supports_mfp(uci) then
				uci:set('wireless', name, 'ieee80211w', mfp_val)
			else
